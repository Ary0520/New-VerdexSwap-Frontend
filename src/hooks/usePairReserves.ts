...
export function usePairReserves(pairAddress: string | undefined) {
  const contract = pairAddress ? getPairContract(pairAddress) : null;

  const { data, isLoading } = useReadContract({
    contract: contract!,
    method: 'function getReserves() view returns (uint112 _reserve0, uint112 _reserve1, uint32 timestamp)',
    params: [],
...
export function useLpBalance(pairAddress: string | undefined, account: string | undefined) {
  ...
  const { data, isLoading } = useReadContract({
    ...
    params: [(account ?? '0x0000000000000000000000000000000000000000') as `0x${string}`],
